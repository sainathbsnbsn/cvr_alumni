...
$select='SELECT * FROM add_events WHERE status=1 ORDER BY `id` DESC ';
...
if(mysqli_query($db,$drop)){ 
$end="UPDATE add_events set status='0' WHERE id='$id'";
mysqli_query($db,$end);
header('location:end_event.php');
}
}
?>
  ...
  <div class="container">
    <div class="row">
      ...
      <div class="title text-center wow fadeInDown">
        <h2>Active Event<span class="color">s</span></h2>
      ...
      <div class="clearfix">
        ...
        <h3 style="text-align: center;">No Active Events</h3>
      <?php }
      else{
      ?>
        ...
        <img src="image/<?php echo $row['event_image']; ?>"  alt="amazing caves coverimage" class="img-fluid" width="100%" height="auto">
            ...
            <input type="hidden" name="id" value="<?php echo $row['id']; ?>" style="display: none;">
              ...
              <p > Registration started on <?php echo $row['date']; ?></p>

              <p ><?php 
              	$nfs="SELECT * FROM $t";
              	$no=mysqli_query($db,$nfs);

              	$cnt=mysqli_num_rows($no);
               ?>
               	Number of Students registered are <?php echo $cnt; ?>
               </p>
              <div id="cf-submit">
					...
					<form action="end_event.php" method="post">
						      <input type="hidden" name="id" value="<?php echo $row['id']; ?>" style="display: none;">
						      <input type="hidden" name="title" value="<?php echo $row['title']; ?>" style="display: none;">
							<div id="cf-submit">
					<input type="submit" name="end" id="submit" class="btn btn-transparent" value="End Event" style="background-color: #57cbcc;">
					</div>
				</form>
